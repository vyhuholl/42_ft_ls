...
/*   Updated: 2019/07/21 13:21:24 by sghezn           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int     ft_get_width(t_file *files, int width[7])
{
    ...
    while (files)
    {
        width[0] = ft_max(ft_nbrlen(files->stats.st_blocks), width[0]);
        width[1] = ft_max(ft_nbrlen(files->stats.st_nlink), width[1]);
        width[2] = ft_max(ft_strlen(getpwuid(files->stats.st_uid)->pw_name), width[2]);
        width[3] = ft_max(ft_strlen(getgrgid(files->stats.st_gid)->gr_name), width[2]);
        if (!S_ISCHR(files->stats.st_mode))
            width[4] = ft_max(ft_nbrlen(files->stats.st_size), width[4]);
        else
        {
            width[5] = ft_max(ft_nbrlen(major(files->stats.st_rdev)), width[5]);
            width[6] = ft_max(ft_nbrlen(minor(files->stats.st_rdev)), width[6]);
            width[4] = ft_max(width[5] + width[6] + 2, width[4]);
        }
        blocks += files->stats.st_blocks;
        files = files->next;
    }
    width[5] = ft_max(width[4] - width[6] - 1, width[5]);
    return (blocks);    
}
...
void    ft_print_long_one(t_file *file, int flags, int width[7])
{
    ...
    ft_print_chmod(file);
    ft_printf(" %*d %-*s  %-*s", width[1], file->stats.st_nlink,
